...
function validate(values) {
  ...
  if (!values.name) {
    ...
    errors.name = 'Field cannot contain special characters: |\{}[];:?<>=+()*^%$#@!~';
  }
...
export default function ReviewForm({ product }) {
  ...
  async function handleSubmit(event) {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const result = await fetch(`${faustConfig.wpUrl}/wp-json/tecom/v1/create-review`, {
        method: 'POST',
        headers: {
    ...
    <section className={styles.reviewForm}>
    {
      ...
      ? (
        <>
          ...
          <a href="#" onClick={handleClickDeactivate} style={{ float: 'right' }}>Cancel</a>
          <div className="row">
            <div className="column column-50">
              ...
              />
              <h6 className={styles.productBrand}>{product.brand?.node?.name}</h6>
              ...
              <form action="/send-data-here" method="post" onSubmit={handleSubmit}>
                {
                  ...
                  ? (
                    <div className={styles.errorMessage}>
                      There was an error submitting the review:<br />
                      {submitResponse.message}
                    </div>
                  )
                  : null
                }
                <div className={classNames([styles.formField, errors.rating ? styles.formFieldError : ''])}>
                  ...
                  {
                    errors.rating
                    ? <div className={styles.formErrorMessage}>{errors.rating}</div>
                    : null
                  }
                </div>

                <div className={classNames([styles.formField, errors.name ? styles.formFieldError : ''])}>
                  <label htmlFor="name">Name</label>
                  <input type="text" id="name" name="name" maxLength="255" value={values.name} onChange={handleChange} />
                  {
                    errors.name
                ...
                <div className={classNames([styles.formField, errors.title ? styles.formFieldError : ''])}>
                  <label htmlFor="subject">Review Subject</label>
                  <input type="text" id="subject" name="title" maxLength="255" value={values.title} onChange={handleChange} />
                  {
                    errors.title
                    ? <div className={styles.formErrorMessage}>{errors.title}</div>
                    : null
                  }
                </div>

                <div className={classNames([styles.formField, errors.text ? styles.formFieldError : ''])}>
                ...
                <button className={styles.button} type="submit" disabled={submitResponse.isLoading}>Submit Review</button>
              </form>
            </div>
          </div>
        </>
      ...
      : (
        ...
        : <a href="#" onClick={handleClickActivate}>Write a Review</a>
      )
    }
    </section>
  );
